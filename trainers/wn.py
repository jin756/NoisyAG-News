...
class WN_Trainer(Trainer):
    def __init__(self, args, logger, log_dir, model_config, full_dataset, random_state):
        super(WN_Trainer, self).__init__(args, logger, log_dir, model_config, full_dataset, random_state)
    ...
    def train(self, args, logger, full_dataset):
        ...
        logger.info('WN Trainer: training started')
        ...
        for idx in tqdm(range(num_training_steps),desc=f'{time.strftime("%m-%d %H:%M")} training - '):
            ce_loss_mean = 0.0
            ...
            for i in range(args.gradient_accumulation_steps):
                ...
                # if idx %320  == 0 and idx != 0 and saveFlag:
            ...
            # gap = 200
            #
            # if idx % gap == 0 and idx > 5000 :
            #     logger.info("save mpdel")
            #     logger.info("idx / gap "+ str(idx%gap))
            #     model_path = "bert_model_weights"+ str(idx//100) + ".bin"
            #     torch.save(model.state_dict(), model_path)
            #     logger.info("save mode at : bert_model_weights_" + str(idx//100) + ".bin")
            ...
            if self.train_eval_fre(args, global_step):
                ...
                logger.info(str(global_step// args.eval_freq) + "-train/loss/val_c_loss-: " + str(train_score['val_c_loss']))
                logger.info(str(global_step// args.eval_freq) +"-train/loss/val_n_loss-: " + str(train_score['val_n_loss']))
                logger.info(str(global_step// args.eval_freq) +"-train/score/val_c_acc-: ||" + str(train_score['score_dict_c']['accuracy']) +"||")
                logger.info(str(global_step// args.eval_freq) +"-train/score/val_n_acc-: ||" + str(train_score['score_dict_n']['accuracy']) +"||")
            ...
            if early_stopper.early_stop:
                ...
                # logger.info(str(global_step// args.eval_freq) + "-eval/loss/val_c_loss-: " + str(val_score['val_c_loss']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/loss/val_n_loss-: " + str(val_score['val_n_loss']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/score/val_c_loss-: " + str(val_score['score_dict_c']['accuracy']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/score/val_n_acc-: " + str(val_score['score_dict_n']['accuracy']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/score/test_acc-: " + str(test_score['score_dict']['accuracy']))

                # train_score = self.eval_model_with_both_train_labels(args,model, nl_bucket, device, logger,idx,savePath,fast_mode=args.fast_eval)
                # logger.info(str(global_step// args.eval_freq) + "-eval/loss/train_c_loss-: " + str(train_score['val_c_loss']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/loss/train_n_loss-: " + str(train_score['val_n_loss']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/score/train_c_loss-: " + str(train_score['score_dict_c']['accuracy']))
                # logger.info(str(global_step// args.eval_freq) +"-eval/score/train_n_acc-: " + str(train_score['score_dict_n']['accuracy']))
