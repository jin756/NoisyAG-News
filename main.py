...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='IMDB', choices=['SST-2', 'AG_News',
        'TREC', 'IMDB', 'Yelp-5','Yoruba', 'Hausa',
       'AG_News_MajorityVote','AG_News_GT_Sample4000','randomLabel','worstLabel','AG_News_GT_Sample50000'])
    ...
    parser.add_argument('--trainer_name', type=str, default='bert_wn',
                        choices=['bert_wn', 'bert_ct', 'bert_cm', 'bert_cmgt',
                                  'bert_smoothing','bert_btls','bert_rweight'],
    ...
    parser.add_argument('--noise_type', default='uniform_m',
                        choices=['uniform_m', 'sflip','uniform','Wmatrix_Ins','NoneNoise'],
    ...
    num_classes_map = {'AG_News': 4, 'TREC': 6, 'SST-2': 2, 'IMDB': 2, 'Yelp-5': 5,
                       'Yoruba':7, 'Hausa':5,'AG_News_MajorityVote':4,
                       'AG_News_GT_Sample4000':4,'randomLabel':4,'worstLabel':4,'AG_News_GT_Sample50000':4}

    logger.info(f'log dir: {log_dir}')
    num_classes = num_classes_map[args.dataset]
    r_state = np.random.RandomState(args.noisy_label_seed)
    # 加数据集需要修改的代码
    if args.dataset in ['SST-2', 'AG_News', 'TREC', 'IMDB', 'Yelp-5','AG_News_MajorityVote',
                        'AG_News_GT_Sample4000','AG_News_GT_Sample50000','randomLabel','worstLabel']:
        ...
        if args.dataset in ['SST-2', 'AG_News', 'TREC', 'Yelp-5','AG_News_MajorityVote','AG_News_GT_Sample4000',
                            'AG_News_GT_Sample50000','randomLabel','worstLabel']:
            has_val = False
            has_ul = False
