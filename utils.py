...
from trainers.bert_btls_trainer import BertBTLS_Trainer
from trainers.bert_rweight_trainer import BertRWEIGHT_Trainer
...
def create_trainer(args, logger, log_dir, model_config, full_dataset, random_state):
    ...
    elif args.trainer_name == 'bert_btls':
        trainer = BertBTLS_Trainer(args, logger, log_dir, model_config, full_dataset, random_state)
    elif args.trainer_name == 'bert_rweight':
        trainer = BertRWEIGHT_Trainer(args, logger, log_dir, model_config, full_dataset, random_state)
    else:
...
def create_log_path(log_root, args, starting_time):
    ...
    # 黄鸿飞修改
    suffix += f'_{args.trainer_name}_{args.dataset}_{args.noise_type}_nle{args.noise_level}'
